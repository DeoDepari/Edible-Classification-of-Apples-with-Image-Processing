# -*- coding: utf-8 -*-
"""
Created on Sun May 31 13:59:54 2020
...
radius=25
n_points=8*radius
METHOD="ror" #uniform,nri_uniform, #default, ror
...
data17_lbp_hist,data18_lbp_hist,data19_lbp_hist,data20_lbp_hist),axis=0).astype(np.float32)


trainTest=np.concatenate((data1_lbp_hist,data2_lbp_hist,data3_lbp_hist,data4_lbp_hist),axis=0).astype(np.float32)
print("Hasil Klasifikasi data1-data4")

print("Neighbors Check=",3)


#Target
responses1=np.array([1,2,2,1,1,2,1,1,1,1,1,2,3,3,2,3]).astype(np.float32)

#kNN
knn=KNeighborsClassifier(n_neighbors=3) #define K=3
knn.fit(trainData,responses1)
res = knn.predict(trainTest)
print("Klasifikasi=",res)
